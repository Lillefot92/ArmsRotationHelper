-- ============================================================
-- Arms Rotation Helper - Diagnostics.lua
...
local EVENT_LABELS = {
    ...
    MAINHAND_SWING = "mainhand-swing",
    REPLACEMENT_SWING = "queued-swing-replacement",
    SLAM_CAST = "slam-cast-complete",
    EXTRA_ATTACK_IGNORED = "extra-attack-ignored",
    EARLY_SWING_IGNORED = "early-swing-ignored",
    SWING_SPEED = "swing-speed-change",
...
local function CaptureMetadata()
    ...
    local improvedSlam = false
    if ns.IsImprovedSlamReady then
        local ok, result = pcall(ns.IsImprovedSlamReady)
        improvedSlam = ok and result == true
    end
    ...
    return {
        ...
        weaponSpeed = Number(state.mainhandSpeed, 0),
        stance = Number(state.stance, 0),
        improvedSlam = improvedSlam,
        improvedDemoRank = Number(state.improvedDemoShoutRank, 0),
        improvedDemoMaxRank = Number(state.improvedDemoShoutMaxRank, 5),
        mode = CleanText(db.mode or "auto"),
        assignedShout = CleanText(db.assignedShout or "battle"),
        stanceAdvice = db.stanceAdvice == true,
        maintainSunder = db.maintainSunder == true,
...
local function CompleteRecording(now)
    ...
    runtime.stoppedAt = now
    RefreshSettings()
    print("|cff4477ffArms Rotation Helper|r: Diagnostic recording complete. "
        .. "Use /arh report to copy it.")
    return true
end
...
function ns.Diagnostics_Sample(snapshot)
    ...
    local state = ns.state or {}
    local swingRemaining = ns.GetSwingRemaining
        and ns.GetSwingRemaining() or 0
    local slamWindowOpen = ns.IsSlamWindowOpen
        and ns.IsSlamWindowOpen() or false
    local gcdRemaining = ns.GetGCDRemaining
        and ns.GetGCDRemaining() or 0
    local signature = SnapshotSignature(snapshot, state, slamWindowOpen)
    ...
    Append({
        ...
        stance = Number(state.stance, 0),
        swing = Number(swingRemaining, 0),
        slamWindow = slamWindowOpen,
        gcd = Number(gcdRemaining, 0),
...
function ns.Diagnostics_GetReport()
    ...
    if not metadata then
        return table.concat({
            "Arms Rotation Helper diagnostic report",
            "No recording is available.",
            "",
            "Use /arh record, fight normally for up to 60 seconds, then "
                .. "use /arh report.",
        }, "\n")
    end
    ...
    local lines = {
        ...
        SettingLine("Weapon speed at start",
            string.format("%.2fs", metadata.weaponSpeed)),
        SettingLine("Stance at start", metadata.stance),
        SettingLine("Improved Slam profile", BooleanText(metadata.improvedSlam)),
        SettingLine("Improved Demoralizing Shout",
            metadata.improvedDemoRank .. "/" .. metadata.improvedDemoMaxRank),
        SettingLine("Target mode", metadata.mode),
        SettingLine("Assigned shout", metadata.assignedShout),
        SettingLine("Stance advice", BooleanText(metadata.stanceAdvice)),
        SettingLine("Maintain Sunder", BooleanText(metadata.maintainSunder)),
        SettingLine("Maintain Demo Shout",
    ...
    for _, entry in ipairs(runtime.entries) do
        ...
        elseif entry.kind == "ability" then
            ...
            table.insert(lines, string.format(
                "%.2f S %s %s %d %.1f %.1f %d %d %.2f %s %.2f %s %s %s %s",
                entry.elapsed,
                entry.main,
                entry.queue,
                entry.rage,
                ...
                entry.stance,
                entry.swing,
                entry.slamWindow and "open" or "closed",
                entry.gcd,
...
local function CreateReportWindow()
    ...
    local scroll = CreateFrame(
        "ScrollFrame",
        "ArmsRotationHelperReportScrollFrame",
        reportFrame,
